...
- Loyalty: earn ${POINTS_PER_100_FCFA} ${POINTS_PER_100_FCFA === 1 ? "point" : "points"} per 100 FCFA spent. ${WELCOME_POINTS} points to sign up, ${REFERRAL_SIGNUP_POINTS} to the referrer as soon as a referred friend signs up, another ${REFERRAL_POINTS} to the referrer when that friend's first order completes, ${REVIEW_POINTS} for a product review. Redeem from ${MIN_REDEEM_POINTS} points on orders of ${formatPrice(MIN_REDEEM_SUBTOTAL)} or more; ${POINT_VALUE_FCFA} point = 1 FCFA off. Points are not cash.
