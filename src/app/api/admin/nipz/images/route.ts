...
export async function POST(request: Request) {
  ...
  const count = await prisma.nipzImage.count();
  if (count >= NIPZ_MAX_IMAGES) {
    return jsonError(
      `Remove an image first — ${NIPZ_MAX_IMAGES} is the maximum.`,
      400,
    );
  }
  ...
  const alt = String(form.get("alt") ?? "").trim() || "Nipz cakes photo";
  ...
  try {
    ...
    if (uploaded.resourceType !== "image") {
      return jsonError("The grid takes images only.", 400);
    }

    const last = await prisma.nipzImage.findFirst({
      orderBy: { sortOrder: "desc" },
      select: { sortOrder: true },
    });

    const image = await prisma.nipzImage.create({
      data: {
        url: uploaded.url,
        alt,
